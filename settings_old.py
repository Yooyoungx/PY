...
CONST_APP_VERSION = "搶票奴隸的工具 2026.04.13 更新"
...
CONST_CENTER = "center"
CONST_RANDOM = "random"
CONST_SELECT_OPTIONS_DEFAULT = (
    CONST_FROM_TOP_TO_BOTTOM, CONST_FROM_BOTTOM_TO_TOP, CONST_CENTER, CONST_RANDOM)

CONST_OCR_CAPTCH_IMAGE_SOURCE_NON_BROWSER = "NonBrowser"
CONST_OCR_CAPTCH_IMAGE_SOURCE_CANVAS = "canvas"

# 把這裡的常數改回原本的英文，核心程式才看得懂
# CONST_WEBDRIVER_TYPE_SELENIUM = "selenium"
CONST_WEBDRIVER_TYPE_UC = "undetected_chromedriver"
# CONST_WEBDRIVER_TYPE_DP = "DrissionPage"
CONST_WEBDRIVER_TYPE_NODRIVER = "nodriver"
# CONST_WEBDRIVER_TYPE_IBON_AAA = "IBON_AAA"
CONST_WEBDRIVER_TYPE_Ibon_debug = "Ibon_debug"
# CONST_WEBDRIVER_TYPE_Tixcraft = "Tixcraft"
CONST_WEBDRIVER_TYPE_Dogg = "Dogg"
CONST_WEBDRIVER_TYPE_PLG = "PLG"

# ==================== 新增：Webdriver 中英對照字典 ====================
WEBDRIVER_MAPPING = {
    "UC": CONST_WEBDRIVER_TYPE_UC,
    "Nodriver ": CONST_WEBDRIVER_TYPE_NODRIVER,
    "IBON售票專用": CONST_WEBDRIVER_TYPE_Ibon_debug,
    "拓元專用": CONST_WEBDRIVER_TYPE_Dogg,
    "年代/遠大/KKTIX/PLG售票": CONST_WEBDRIVER_TYPE_PLG
}
# ======================================================================
...
SITE_MAPPING = {
    ...
    "Ibon售票系統": "https://ticket.ibon.com.tw/",
    "寬宏售票": "https://kham.com.tw/application/utk13/utk1306_.aspx",
    "年代售票": "https://ticket.com.tw/",
    "udn售票網": "https://tickets.udnfunlife.com/",
    "遠大售票": "https://ticketplus.com.tw/",
    "▃▃▃▃▃▃▃▃▃▃韓國售票系統▃▃▃▃▃▃▃▃▃▃▃": "▃▃▃▃▃▃▃▃▃▃韓國售票系統▃▃▃▃▃▃▃▃▃▃▃",
    "Melon Ticket": "https://tkglobal.melon.com/main/index.htm?langCd=CN",
    "韓國公園": "https://world.nol.com/",
    "▃▃▃▃▃▃▃▃▃▃較少用售票系統▃▃▃▃▃▃▃▃▃▃▃": "▃▃▃▃▃▃▃▃▃▃較少用售票系統▃▃▃▃▃▃▃▃▃▃▃",
    "獨立音樂 (iNDIEVOX)": "https://www.indievox.com/",
    "FunOne Tickets": "https://tickets.funone.io/",
    "全網購票網": "https://www.famiticket.com.tw",
    "Ticketmaster SG": "https://ticketmaster.sg",
    "添翼售票": "https://teamear.tixcraft.com/",
    "富邦PLG籃球": "https://tix.fubonbraves.com/UTK0101_",
    "Billboard": "https://www.billboardlivetaipei.tw/tw",
    "台鋼雄鷹棒球": "https://ticket.tsghawks.com/",
    "富邦棒球": "https://guardians.fami.life/UTK0101_",
    "▃▃▃▃▃▃▃▃未更新無法使用▃▃▃▃▃▃▃▃▃": "▃▃▃▃▃▃▃▃未更新無法使用▃▃▃▃▃▃▃▃▃",
    "城市售票網 (URBTIX)": "http://www.urbtix.hk/",
    "Cityline (買飛)": "https://www.cityline.com/",
    "快達票 (HKTicketing)": "https://hotshow.hkticketing.com/",
    "澳門銀河": "https://ticketing.galaxymacau.com/",
    "Ticketek AU": "http://premier.ticketek.com.au"

}

# 下拉選單只抓取字典的「鍵 (Keys)」，顯示乾淨的中文名稱
CONST_SUPPORTED_SITES = list(SITE_MAPPING.keys())
...
def get_default_config():
    ...
    CONST_EXCLUDE_DEFAULT = "\"輪椅\",\"身障\",\"身心 障礙\",\"Restricted View\",\"Wheelchair\",\"燈柱遮蔽\",\"視線不完整\",\"愛心\""
...
# 修改前：永遠讀取 CONST_MAXBOT_CONFIG_FILE
# 修改後：增加一個 filename 參數
def load_json(filename="settings.json"):
    app_root = util.get_app_root()
    config_filepath = os.path.join(app_root, filename)
...
def btn_restore_defaults_clicked():
    ...
    messagebox.showinfo(
        translate[language_code]["restore_defaults"], translate[language_code]["done"])
...
def btn_save_act(slience_mode=False):
    ...
    # 動態抓取下拉選單裡的檔名
    global combo_profile
    global current_config_filename
    if 'combo_profile' in globals():
        current_config_filename = combo_profile.get().strip()
        if not current_config_filename.endswith(".json"):
            current_config_filename += ".json"
    else:
        current_config_filename = "settings.json"

    config_filepath = os.path.join(app_root, current_config_filename)

    # ================= [新增] 建立橋樑檔案，告訴 Dogg.py 要讀誰 =================
    try:
        pointer_filepath = os.path.join(app_root, "MAXBOT_CURRENT_PROFILE.txt")
        with open(pointer_filepath, "w", encoding="utf-8") as f:
            f.write(current_config_filename)
    except Exception as e:
        print("寫入設定檔指標失敗:", e)
    # ===========================================================================
    ...
    if is_all_data_correct:
        if combo_homepage.get().strip() == "":
            ...
            selected_name = combo_homepage.get().strip()
            # 查表：把介面上的乾淨名稱，轉換回機器人需要的真實網址
            homepage_domain = SITE_MAPPING.get(selected_name, selected_name)
    ...
    if is_all_data_correct:
        if combo_browser.get().strip() == "":
            ...
            messagebox.showerror(
                "Error", "Please select a browser: chrome or firefox")
        else:
    ...
    if is_all_data_correct:
        if combo_ticket_number.get().strip() == "":
            ...
            config_dict["ticket_number"] = int(
                combo_ticket_number.get().strip())

    if is_all_data_correct:
        config_dict["kktix"]["auto_press_next_step_button"] = bool(
            chk_state_auto_press_next_step_button.get())
        config_dict["kktix"]["auto_fill_ticket_number"] = bool(
            chk_state_auto_fill_ticket_number.get())

        config_dict["kktix"]["max_dwell_time"] = int(
            txt_max_dwell_time.get().strip())
        if config_dict["kktix"]["max_dwell_time"] > 0:
            if config_dict["kktix"]["max_dwell_time"] < 15:
                config_dict["kktix"]["max_dwell_time"] = 15

        config_dict["cityline"]["cityline_queue_retry"] = bool(
            chk_state_cityline_queue_retry.get())

        config_dict["date_auto_select"]["enable"] = bool(
            chk_state_date_auto_select.get())
        config_dict["date_auto_select"]["mode"] = combo_date_auto_select_mode.get(
        ).strip()
        ...
        config_dict["tixcraft"]["pass_date_is_sold_out"] = bool(
            chk_state_pass_date_is_sold_out.get())
        config_dict["tixcraft"]["auto_reload_coming_soon_page"] = bool(
            chk_state_auto_reload_coming_soon_page.get())
        ...
        user_guess_string = util.format_config_keyword_for_json(
            user_guess_string)
        ...
        idle_keyword_second = util.format_config_keyword_for_json(
            idle_keyword_second)

        resume_keyword_second = txt_resume_keyword_second.get(
            "1.0", END).strip()
        resume_keyword_second = util.format_config_keyword_for_json(
            resume_keyword_second)
        ...
        if is_all_data_correct:
            if len(area_keyword) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["area_keyword"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(keyword_exclude) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["keyword_exclude"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(user_guess_string) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["user_guess_string"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(remote_url) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["remote_url"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(idle_keyword) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["idle_keyword"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(resume_keyword) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["resume_keyword"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(idle_keyword_second) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["idle_keyword_second"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            if len(resume_keyword_second) > 0:
                try:
                    ...
                    print(exc)
                    messagebox.showinfo(
                        translate[language_code]["save"], "Error:" + translate[language_code]["resume_keyword_second"])
                    is_all_data_correct = False
        ...
        if is_all_data_correct:
            ...
            txt_idle_keyword.insert(
                "1.0", config_dict["advanced"]["idle_keyword"].strip())
            txt_resume_keyword.insert(
                "1.0", config_dict["advanced"]["resume_keyword"].strip())
            txt_idle_keyword_second.insert(
                "1.0", config_dict["advanced"]["idle_keyword_second"].strip())
            txt_resume_keyword_second.insert(
                "1.0", config_dict["advanced"]["resume_keyword_second"].strip())

    if is_all_data_correct:
        config_dict["refresh_datetime"] = txt_refresh_datetime.get().strip()
        config_dict["area_auto_select"]["enable"] = bool(
            chk_state_area_auto_select.get())
        config_dict["area_auto_select"]["mode"] = combo_area_auto_select_mode.get(
        ).strip()

        config_dict["advanced"]["play_sound"]["ticket"] = bool(
            chk_state_play_ticket_sound.get())
        config_dict["advanced"]["play_sound"]["order"] = bool(
            chk_state_play_order_sound.get())
        config_dict["advanced"]["play_sound"]["filename"] = txt_play_sound_filename.get(
        ).strip()

        config_dict["advanced"]["tixcraft_sid"] = txt_tixcraft_sid.get().strip()
        config_dict["advanced"]["ibonqware"] = txt_ibon_ibonqware.get().strip()

        config_dict["advanced"]["facebook_account"] = txt_facebook_account.get(
        ).strip()
        ...
        config_dict["advanced"]["cityline_account"] = txt_cityline_account.get(
        ).strip()
        config_dict["advanced"]["urbtix_account"] = txt_urbtix_account.get().strip()
        config_dict["advanced"]["hkticketing_account"] = txt_hkticketing_account.get(
        ).strip()
        ...
        config_dict["advanced"]["ticketplus_account"] = txt_ticketplus_account.get(
        ).strip()

        config_dict["advanced"]["facebook_password"] = txt_facebook_password.get(
        ).strip()
        ...
        config_dict["advanced"]["cityline_password"] = txt_cityline_password.get(
        ).strip()
        config_dict["advanced"]["urbtix_password"] = txt_urbtix_password.get().strip()
        config_dict["advanced"]["hkticketing_password"] = txt_hkticketing_password.get(
        ).strip()
        ...
        config_dict["advanced"]["ticketplus_password"] = txt_ticketplus_password.get(
        ).strip()
        ...
        config_dict["advanced"]["facebook_password"] = util.encrypt_me(
            config_dict["advanced"]["facebook_password"])
        config_dict["advanced"]["kktix_password"] = util.encrypt_me(
            config_dict["advanced"]["kktix_password"])
        config_dict["advanced"]["fami_password"] = util.encrypt_me(
            config_dict["advanced"]["fami_password"])
        config_dict["advanced"]["cityline_password"] = util.encrypt_me(
            config_dict["advanced"]["cityline_password"])
        config_dict["advanced"]["urbtix_password"] = util.encrypt_me(
            config_dict["advanced"]["urbtix_password"])
        config_dict["advanced"]["hkticketing_password"] = util.encrypt_me(
            config_dict["advanced"]["hkticketing_password"])
        config_dict["advanced"]["kham_password"] = util.encrypt_me(
            config_dict["advanced"]["kham_password"])
        config_dict["advanced"]["ticket_password"] = util.encrypt_me(
            config_dict["advanced"]["ticket_password"])
        config_dict["advanced"]["udn_password"] = util.encrypt_me(
            config_dict["advanced"]["udn_password"])
        config_dict["advanced"]["ticketplus_password"] = util.encrypt_me(
            config_dict["advanced"]["ticketplus_password"])

        # ==================== [新增] 儲存 Telegram 設定 ====================
        config_dict["advanced"]["telegram_enable"] = bool(
            chk_state_telegram_enable.get())
        config_dict["advanced"]["telegram_bot_token"] = txt_telegram_bot_token.get(
        ).strip()
        config_dict["advanced"]["telegram_chat_id"] = txt_telegram_chat_id.get(
        ).strip()
        # =================================================================

        config_dict["advanced"]["chrome_extension"] = bool(
            chk_state_chrome_extension.get())
        config_dict["advanced"]["disable_adjacent_seat"] = bool(
            chk_state_adjacent_seat.get())
        config_dict["advanced"]["hide_some_image"] = bool(
            chk_state_hide_some_image.get())
        config_dict["advanced"]["block_facebook_network"] = bool(
            chk_state_block_facebook_network.get())

        config_dict["ocr_captcha"] = {}
        config_dict["ocr_captcha"]["enable"] = bool(
            chk_state_ocr_captcha.get())
        config_dict["ocr_captcha"]["beta"] = bool(
            chk_state_ocr_captcha_ddddocr_beta.get())
        config_dict["ocr_captcha"]["force_submit"] = bool(
            chk_state_ocr_captcha_force_submit.get())
        config_dict["ocr_captcha"]["image_source"] = combo_ocr_captcha_image_source.get(
        ).strip()
        # ibon不強制nodirver
        ...
        # 將介面上的中文名稱，轉換回底層需要的英文代號存檔
        selected_webdriver = combo_webdriver_type.get().strip()
        config_dict["webdriver_type"] = WEBDRIVER_MAPPING.get(
            selected_webdriver, selected_webdriver)

        config_dict["advanced"]["headless"] = bool(chk_state_headless.get())
        # config_dict["advanced"]["verbose"] = bool(chk_state_verbose.get())

        config_dict["advanced"]["auto_guess_options"] = bool(
            chk_state_auto_guess_options.get())

        # === 新增儲存邏輯 ===
        config_dict["advanced"]["disable_auto_update_config"] = bool(
            chk_state_disable_auto_update_config.get())
        # ===================
        #
        #
        config_dict["advanced"]["auto_reload_page_interval"] = float(
            txt_auto_reload_page_interval.get().strip())

        # ==================== 新增儲存邏輯 ====================
        config_dict["advanced"]["auto_reload_random_mode"] = bool(
            chk_state_auto_reload_random_mode.get())

        config_dict["advanced"]["auto_reload_random_range"] = txt_auto_reload_random_range.get(
        ).strip()
        # ====================================================

        config_dict["advanced"]["reset_browser_interval"] = int(
            txt_reset_browser_interval.get().strip())
        config_dict["advanced"]["proxy_server_port"] = txt_proxy_server_port.get(
        ).strip()
    ...
    if is_all_data_correct:
        if not slience_mode:
            # 彈出視窗時，預設名稱帶入當前選中的檔名
            file_to_save = asksaveasfilename(initialdir=app_root, initialfile=current_config_filename, defaultextension=".json", filetypes=[
...
def launch_maxbot():
    ...
    if 'combo_webdriver_type' in globals():
        selected_webdriver = combo_webdriver_type.get().strip()
        # 把中文轉回英文代號，再丟給底層執行
        webdriver_type = WEBDRIVER_MAPPING.get(
            selected_webdriver, selected_webdriver)

    script_name = "chrome_tixcraft"
    if webdriver_type == CONST_WEBDRIVER_TYPE_NODRIVER:
        script_name = "nodriver_tixcraft"
    # elif webdriver_type == CONST_WEBDRIVER_TYPE_IBON_AAA:
    #    script_name = "IBON_AAA"     # 對應 / 同層的
    elif webdriver_type == CONST_WEBDRIVER_TYPE_Ibon_debug:
        script_name = "Ibon_debug"
    # elif webdriver_type == CONST_WEBDRIVER_TYPE_Tixcraft:
    #    script_name = "Tixcraft"
    elif webdriver_type == CONST_WEBDRIVER_TYPE_Dogg:
        script_name = "Dogg"
    elif webdriver_type == CONST_WEBDRIVER_TYPE_PLG:
        script_name = "PLG"
    ...
    # 抓取目前的檔名
    global current_config_filename
    # 注意：原本這裡傳給 util.launch_maxbot 的第二個參數是空的 ""
    # 我們要把它改成當前選中的 .json 檔名！
    threading.Thread(target=util.launch_maxbot, args=(
        script_name, current_config_filename, "", "", "", window_size,)).start()
    ...
    if len(remote_url) > 0:
        ...
        if len(url_array) > 0:
            ...
            for each_url in url_array:
                ...
                is_write_to_file = util.save_url_to_file(
                    each_url, CONST_MAXBOT_ANSWER_ONLINE_FILE, force_write=force_write)
                if is_write_to_file:
                    break
    show_preview_text()
...
def applyNewLanguage():
    ...
    lbl_refresh_datetime.config(
        text=translate[language_code]["refresh_datetime"])

    lbl_auto_press_next_step_button.config(
        text=translate[language_code]["auto_press_next_step_button"])
    lbl_auto_fill_ticket_number.config(
        text=translate[language_code]["auto_fill_ticket_number"])
    lbl_user_guess_string_description.config(
        text=translate[language_code]["user_guess_string"])
    lbl_user_guess_string.config(
        text=translate[language_code]["local_dictionary"])

    lbl_date_auto_select.config(
        text=translate[language_code]["date_auto_select"])
    lbl_date_auto_select_mode.config(
        text=translate[language_code]["date_select_order"])
    lbl_date_keyword.config(text=translate[language_code]["date_keyword"])
    lbl_area_auto_select.config(
        text=translate[language_code]["area_auto_select"])
    lbl_area_auto_select_mode.config(
        text=translate[language_code]["area_select_order"])
    lbl_area_keyword.config(text=translate[language_code]["area_keyword"])
    lbl_keyword_exclude.config(
        text=translate[language_code]["keyword_exclude"])
    lbl_keyword_usage.config(text=translate[language_code]["keyword_usage"])
    lbl_pass_date_is_sold_out.config(
        text=translate[language_code]["pass_date_is_sold_out"])
    lbl_auto_reload_coming_soon_page.config(
        text=translate[language_code]["auto_reload_coming_soon_page"])
    lbl_ocr_captcha.config(text=translate[language_code]["ocr_captcha"])
    lbl_ocr_captcha_ddddocr_beta.config(
        text=translate[language_code]["ocr_captcha_ddddocr_beta"])
    lbl_ocr_captcha_force_submit.config(
        text=translate[language_code]["ocr_captcha_force_submit"])
    lbl_ocr_captcha_image_source.config(
        text=translate[language_code]["ocr_captcha_image_source"])
    lbl_webdriver_type.config(text=translate[language_code]["webdriver_type"])
    lbl_chrome_extension.config(
        text=translate[language_code]["chrome_extension"])
    lbl_adjacent_seat.config(
        text=translate[language_code]["disable_adjacent_seat"])
    lbl_hide_some_image.config(
        text=translate[language_code]["hide_some_image"])
    lbl_block_facebook_network.config(
        text=translate[language_code]["block_facebook_network"])

    lbl_hide_some_image_recommand.config(
        text=translate[language_code]["recommand_enable"])
    lbl_block_facebook_network_recommand.config(
        text=translate[language_code]["recommand_enable"])

    lbl_auto_reload_page_interval.config(
    ...
    lbl_cityline_queue_retry.config(
        text=translate[language_code]["cityline_queue_retry"])
    lbl_reset_browser_interval.config(
        text=translate[language_code]["reset_browser_interval"])
    lbl_proxy_server_port.config(
        text=translate[language_code]["proxy_server_port"])
    ...
    lbl_online_dictionary_preview.config(
        text=translate[language_code]["preview"])
    lbl_auto_guess_options.config(
        text=translate[language_code]["auto_guess_options"])
    ...
    lbl_idle_keyword_second.config(
        text=translate[language_code]["idle_keyword_second"])
    lbl_resume_keyword_second.config(
        text=translate[language_code]["resume_keyword_second"])

    chk_auto_press_next_step_button.config(
        text=translate[language_code]["enable"])
    ...
    chk_auto_reload_coming_soon_page.config(
        text=translate[language_code]["enable"])
    ...
    chk_ocr_captcha_ddddocr_beta.config(
        text=translate[language_code]["enable"])
    chk_ocr_captcha_force_submit.config(
        text=translate[language_code]["enable"])
    ...
    lbl_facebook_account.config(
        text=translate[language_code]["facebook_account"])
    ...
    lbl_cityline_account.config(
        text=translate[language_code]["cityline_account"])
    lbl_urbtix_account.config(text=translate[language_code]["urbtix_account"])
    lbl_hkticketing_account.config(
        text=translate[language_code]["hkticketing_account"])
    ...
    lbl_ticketplus_account.config(
        text=translate[language_code]["ticketplus_account"])
    ...
    lbl_save_password_alert.config(
        text=translate[language_code]["save_password_alert"])

    lbl_play_ticket_sound.config(
        text=translate[language_code]["play_ticket_sound"])
    lbl_play_order_sound.config(
        text=translate[language_code]["play_order_sound"])
    lbl_play_sound_filename.config(
        text=translate[language_code]["play_sound_filename"])
    ...
    btn_restore_defaults.config(
        text=translate[language_code]["restore_defaults"])
...
def callbackProfileOnChange(event):
    global combo_profile
    global current_config_filename
    global root
    ...
    # 重新讀取該設定檔並刷新整個 GUI 介面
    _, new_config_dict = load_json(current_config_filename)
    load_GUI(root, new_config_dict)
...
def showHideBlocks():
    ...
    global combo_homepage
    global combo_webdriver_type  # <--- 將宣告統一拉到最上面，保證不會報錯！

    new_homepage = ""
    selected_site_name = ""

    if 'combo_homepage' in globals():
        selected_site_name = combo_homepage.get().strip()
        # 查表法：轉回真實網址供下方判斷 KKTIX
        new_homepage = SITE_MAPPING.get(selected_site_name, selected_site_name)
    ...
    if 'combo_webdriver_type' in globals():
        # 1. 判斷是否為「拓元售票」
        if selected_site_name == "拓元售票":
            combo_webdriver_type.set("拓元專用")
            combo_webdriver_type.config(state="disabled")

        # 2. 判斷是否為「ibon售票系統」
        elif selected_site_name == "Ibon售票系統":
            combo_webdriver_type.set("IBON售票專用")
            combo_webdriver_type.config(state="disabled")

        # 3. 其他網站 (預設為 PLG.PY)
        else:
            combo_webdriver_type.set("年代/遠大/KKTIX/PLG售票")
            combo_webdriver_type.config(state="disabled")
    # ==================================================================
    ...
    if 'frame_group_kktix' in globals():
        if show_block_index == BLOCK_STYLE_KKTIX:
            frame_group_kktix.grid(
                column=0, row=frame_group_kktix_index, padx=UI_PADDING_X)
            frame_group_tixcraft.grid_forget()
        else:
            frame_group_tixcraft.grid(
                column=0, row=frame_group_tixcraft_index, padx=UI_PADDING_X)
...
def showHideOcrCaptchaWithSubmit():
    ...
    if is_ocr_captcha_enable:
        # show.
        lbl_ocr_captcha_force_submit.grid(
            column=0, row=ocr_captcha_force_submit_index, sticky=E)
        chk_ocr_captcha_force_submit.grid(
            column=1, row=ocr_captcha_force_submit_index, sticky=W)

        lbl_ocr_captcha_ddddocr_beta.grid(
            column=0, row=ocr_captcha_force_submit_index-1, sticky=E)
        chk_ocr_captcha_ddddocr_beta.grid(
            column=1, row=ocr_captcha_force_submit_index-1, sticky=W)
    else:
        # hide
...
def showHideTixcraftBlocks():
    ...
    if is_date_set_to_enable:
        # show
        lbl_date_auto_select_mode.grid(
            column=0, row=date_auto_select_mode_index, sticky=E)
        combo_date_auto_select_mode.grid(
            column=1, row=date_auto_select_mode_index, sticky=W)
...
def showHideAreaBlocks():
    ...
    if is_area_set_to_enable:
        # show
        lbl_area_auto_select_mode.grid(
            column=0, row=area_auto_select_mode_index, sticky=E)
        combo_area_auto_select_mode.grid(
            column=1, row=area_auto_select_mode_index, sticky=W)
        ...
        lbl_keyword_exclude.grid(
            column=0, row=keyword_exclude_index, sticky=E+N)
...
def on_homepage_configure(event):
    ...
    width = font.measure(CONST_SUPPORTED_SITES[len(
        CONST_SUPPORTED_SITES)-1] + "0") - event.width
    style = ttk.Style()
...
def PreferenctTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    # 設定檔選擇區域
    import glob
    # 自動抓取資料夾內所有以 settings 開頭的 json 檔案
    profile_list = glob.glob("settings*.json")
    if "settings.json" not in profile_list:
        profile_list.insert(0, "settings.json")

    global current_config_filename
    if 'current_config_filename' not in globals():
        current_config_filename = "settings.json"

    group_row_count = 0  # 確保它是第一行

    Label(frame_group_header, text="設定檔選擇").grid(
        column=0, row=group_row_count, sticky=E)
    global combo_profile
    combo_profile = ttk.Combobox(frame_group_header, width=30)
    combo_profile['values'] = profile_list
    combo_profile.set(current_config_filename)
    combo_profile.grid(column=1, row=group_row_count, sticky=W)

    # 當切換選單時，自動重新載入 GUI 上的數據
    def on_profile_change(event):
        global current_config_filename, root
        current_config_filename = combo_profile.get()
        _, new_config = load_json(current_config_filename)
        load_GUI(root, new_config)  # 重新刷掉舊介面
    ...
    global lbl_homepage
    lbl_homepage = Label(frame_group_header,
                         text=translate[language_code]['homepage'])
    ...
    saved_url = config_dict.get("homepage", "")
    display_name = saved_url  # 預設為原網址
    for name, url in SITE_MAPPING.items():
        if url == saved_url:
            display_name = name
            break
    combo_homepage.set(display_name)
    ...
    global lbl_ticket_number
    lbl_ticket_number = Label(
        frame_group_header, text=translate[language_code]['ticket_number'])
    ...
    '''
    combo_ticket_number = ttk.Combobox(
        frame_group_header, state="readonly", width=30)
    combo_ticket_number['values'] = (
    ...
    frame_group_header.grid(column=0, row=row_count,
                            sticky=W, padx=UI_PADDING_X)

    group_row_count += 1

    global lbl_refresh_datetime
    lbl_refresh_datetime = Label(
        frame_group_header, text=translate[language_code]['refresh_datetime'])
    # 1. 註解掉 Label 的 grid (不讓文字顯示)
    # lbl_refresh_datetime.grid(column=0, row=group_row_count, sticky=E)

    global txt_refresh_datetime
    txt_refresh_datetime_value = StringVar(
        frame_group_header, value=str(config_dict["refresh_datetime"]))
    txt_refresh_datetime = Entry(
        frame_group_header, width=30, textvariable=txt_refresh_datetime_value)
    # 2. 註解掉 Entry 的 grid (不讓輸入框顯示)
    # txt_refresh_datetime.grid(column=1, row=group_row_count, sticky=W)
    # 3. 註解掉行數增加 (避免畫面上多出一個空行)
    # group_row_count += 1

    # ==================== 從 AdvancedTab 移過來的區塊 ====================

    # Webdriver 選項判斷邏輯
    webdriver_type_options = (
        CONST_WEBDRIVER_TYPE_UC,
        CONST_WEBDRIVER_TYPE_NODRIVER,
        CONST_WEBDRIVER_TYPE_Ibon_debug,
        CONST_WEBDRIVER_TYPE_Dogg, CONST_WEBDRIVER_TYPE_PLG)
    ...
    # 1. Webdriver 類別
    global lbl_webdriver_type
    lbl_webdriver_type = Label(
        frame_group_header, text=translate[language_code]['webdriver_type'])
    lbl_webdriver_type.grid(column=0, row=group_row_count, sticky=E)

    global combo_webdriver_type
    combo_webdriver_type = ttk.Combobox(
        frame_group_header, state="readonly", width=30)
    # 直接使用字典的中文名稱作為下拉選單的選項
    combo_webdriver_type['values'] = list(WEBDRIVER_MAPPING.keys())

    # 反向查表：讀取 settings.json 時，把英文轉回中文顯示在介面上
    saved_webdriver = config_dict.get(
        "webdriver_type", CONST_WEBDRIVER_TYPE_UC)
    display_webdriver = saved_webdriver
    for name, code in WEBDRIVER_MAPPING.items():
        if code == saved_webdriver:
            display_webdriver = name
            break
    ...
    # 2. 自動刷新頁面間隔
    global lbl_auto_reload_page_interval
    lbl_auto_reload_page_interval = Label(
        frame_group_header, text=translate[language_code]['auto_reload_page_interval'])
    lbl_auto_reload_page_interval.grid(column=0, row=group_row_count, sticky=E)

    global txt_auto_reload_page_interval
    txt_auto_reload_page_interval_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["auto_reload_page_interval"])
    txt_auto_reload_page_interval = Entry(
        frame_group_header, width=30, textvariable=txt_auto_reload_page_interval_value)
    txt_auto_reload_page_interval.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    # 3. 隨機刷新模式
    global lbl_auto_reload_random_mode
    lbl_auto_reload_random_mode = Label(
        frame_group_header, text="啟用隨機刷新(Enable Random)")
    lbl_auto_reload_random_mode.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_auto_reload_random_mode
    chk_state_auto_reload_random_mode = BooleanVar()
    chk_state_auto_reload_random_mode.set(
        config_dict["advanced"].get("auto_reload_random_mode", False))

    global chk_auto_reload_random_mode
    chk_auto_reload_random_mode = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_auto_reload_random_mode)
    chk_auto_reload_random_mode.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    # 4. 隨機增減秒數
    global lbl_auto_reload_random_range
    lbl_auto_reload_random_range = Label(
        frame_group_header, text="隨機增減秒數 (± N秒)")
    lbl_auto_reload_random_range.grid(column=0, row=group_row_count, sticky=E)

    global txt_auto_reload_random_range
    range_val = config_dict["advanced"].get(
        "auto_reload_random_range", "0.5,1.5")
    txt_auto_reload_random_range_value = StringVar(
        frame_group_header, value=range_val)

    txt_auto_reload_random_range = Entry(
        frame_group_header, width=30, textvariable=txt_auto_reload_random_range_value)
    txt_auto_reload_random_range.grid(column=1, row=group_row_count, sticky=W)
    ...
    global lbl_auto_press_next_step_button
    lbl_auto_press_next_step_button = Label(
        frame_group_kktix, text=translate[language_code]['auto_press_next_step_button'])
    lbl_auto_press_next_step_button.grid(
        column=0, row=group_row_count, sticky=E)

    global chk_state_auto_press_next_step_button
    chk_state_auto_press_next_step_button = BooleanVar()
    chk_state_auto_press_next_step_button.set(
        config_dict["kktix"]["auto_press_next_step_button"])

    global chk_auto_press_next_step_button
    chk_auto_press_next_step_button = Checkbutton(
        frame_group_kktix, text=translate[language_code]['enable'], variable=chk_state_auto_press_next_step_button)
    chk_auto_press_next_step_button.grid(
        column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_auto_fill_ticket_number
    lbl_auto_fill_ticket_number = Label(
        frame_group_kktix, text=translate[language_code]['auto_fill_ticket_number'])
    lbl_auto_fill_ticket_number.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_auto_fill_ticket_number
    chk_state_auto_fill_ticket_number = BooleanVar()
    chk_state_auto_fill_ticket_number.set(
        config_dict["kktix"]["auto_fill_ticket_number"])

    global chk_auto_fill_ticket_number
    chk_auto_fill_ticket_number = Checkbutton(
        frame_group_kktix, text=translate[language_code]['enable'], variable=chk_state_auto_fill_ticket_number)
    ...
    global lbl_date_auto_select
    lbl_date_auto_select = Label(
        frame_group_tixcraft, text=translate[language_code]['date_auto_select'])
    ...
    global chk_date_auto_select
    chk_date_auto_select = Checkbutton(
        frame_group_tixcraft, text=translate[language_code]['enable'], variable=chk_state_date_auto_select, command=callbackDateAutoOnChange)
    ...
    global lbl_date_auto_select_mode
    lbl_date_auto_select_mode = Label(
        frame_group_tixcraft, text=translate[language_code]['date_select_order'])
    lbl_date_auto_select_mode.grid(
        column=0, row=date_auto_select_mode_index, sticky=E)

    global combo_date_auto_select_mode
    combo_date_auto_select_mode = ttk.Combobox(
        frame_group_tixcraft, state="readonly", width=30)
    ...
    global lbl_date_keyword
    lbl_date_keyword = Label(frame_group_tixcraft,
                             text=translate[language_code]['date_keyword'])
    lbl_date_keyword.grid(column=0, row=date_keyword_index, sticky=E+N)

    global txt_date_keyword
    txt_date_keyword = Text(frame_group_tixcraft, width=30, height=4)
    txt_date_keyword.grid(column=1, row=group_row_count, sticky=W)
    txt_date_keyword.insert(
        "1.0", config_dict["date_auto_select"]["date_keyword"].strip())

    group_row_count += 1

    global lbl_pass_date_is_sold_out
    lbl_pass_date_is_sold_out = Label(
        frame_group_tixcraft, text=translate[language_code]['pass_date_is_sold_out'])
    lbl_pass_date_is_sold_out.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_pass_date_is_sold_out
    chk_state_pass_date_is_sold_out = BooleanVar()
    chk_state_pass_date_is_sold_out.set(
        config_dict["tixcraft"]["pass_date_is_sold_out"])

    global chk_pass_date_is_sold_out
    chk_pass_date_is_sold_out = Checkbutton(
        frame_group_tixcraft, text=translate[language_code]['enable'], variable=chk_state_pass_date_is_sold_out)
    chk_pass_date_is_sold_out.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_auto_reload_coming_soon_page
    lbl_auto_reload_coming_soon_page = Label(
        frame_group_tixcraft, text=translate[language_code]['auto_reload_coming_soon_page'])
    lbl_auto_reload_coming_soon_page.grid(
        column=0, row=group_row_count, sticky=E)

    global chk_state_auto_reload_coming_soon_page
    chk_state_auto_reload_coming_soon_page = BooleanVar()
    chk_state_auto_reload_coming_soon_page.set(
        config_dict["tixcraft"]["auto_reload_coming_soon_page"])

    global chk_auto_reload_coming_soon_page
    chk_auto_reload_coming_soon_page = Checkbutton(
        frame_group_tixcraft, text=translate[language_code]['enable'], variable=chk_state_auto_reload_coming_soon_page)
    chk_auto_reload_coming_soon_page.grid(
        column=1, row=group_row_count, sticky=W)
    ...
    group_row_count = 0
    # ==================== 允許不連續座位 (獨立出來，放在區域選擇上方) ====================
    global lbl_adjacent_seat
    lbl_adjacent_seat = Label(
        frame_group_area, text=translate[language_code]['disable_adjacent_seat'])
    # 加上 pady 讓格子對齊更有呼吸空間
    lbl_adjacent_seat.grid(column=0, row=group_row_count,
                           sticky=E, pady=3, padx=5)

    global chk_state_adjacent_seat
    chk_state_adjacent_seat = BooleanVar()
    chk_state_adjacent_seat.set(
        config_dict["advanced"]["disable_adjacent_seat"])

    global chk_adjacent_seat
    chk_adjacent_seat = Checkbutton(
        frame_group_area, text=translate[language_code]['enable'], variable=chk_state_adjacent_seat)
    chk_adjacent_seat.grid(column=1, row=group_row_count, sticky=W, pady=3)

    group_row_count += 1
    # =====================================================================
    global lbl_area_auto_select
    lbl_area_auto_select = Label(
        frame_group_area, text=translate[language_code]['area_auto_select'])
    ...
    global chk_area_auto_select
    chk_area_auto_select = Checkbutton(
        frame_group_area, text=translate[language_code]['enable'], variable=chk_state_area_auto_select, command=callbackAreaAutoOnChange)
    ...
    global lbl_area_auto_select_mode
    lbl_area_auto_select_mode = Label(
        frame_group_area, text=translate[language_code]['area_auto_select'])
    lbl_area_auto_select_mode.grid(
        column=0, row=area_auto_select_mode_index, sticky=E)

    global combo_area_auto_select_mode
    combo_area_auto_select_mode = ttk.Combobox(
        frame_group_area, state="readonly", width=30)
    ...
    global lbl_area_keyword
    lbl_area_keyword = Label(
        frame_group_area, text=translate[language_code]['area_keyword'])
    ...
    txt_area_keyword.insert(
        "1.0", config_dict["area_auto_select"]["area_keyword"].strip())

    group_row_count += 1

    global lbl_keyword_exclude
    lbl_keyword_exclude = Label(
        frame_group_area, text=translate[language_code]['keyword_exclude'])
    ...
    global lbl_keyword_usage
    lbl_keyword_usage = Label(
        frame_group_area, text=translate[language_code]['keyword_usage'])
...
def AdvancedTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    global lbl_browser
    lbl_browser = Label(frame_group_header,
                        text=translate[language_code]['browser'])
    ...
    global lbl_language
    lbl_language = Label(frame_group_header,
                         text=translate[language_code]['language'])
    ...
    global lbl_ocr_captcha_image_source
    lbl_ocr_captcha_image_source = Label(
        frame_group_header, text=translate[language_code]['ocr_captcha_image_source'])
    lbl_ocr_captcha_image_source.grid(column=0, row=group_row_count, sticky=E)

    global combo_ocr_captcha_image_source
    combo_ocr_captcha_image_source = ttk.Combobox(
        frame_group_header, state="readonly", width=30)
    combo_ocr_captcha_image_source['values'] = (
        CONST_OCR_CAPTCH_IMAGE_SOURCE_NON_BROWSER, CONST_OCR_CAPTCH_IMAGE_SOURCE_CANVAS)
    combo_ocr_captcha_image_source.set(
        config_dict["ocr_captcha"]["image_source"])
    combo_ocr_captcha_image_source.grid(
        column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_play_ticket_sound
    lbl_play_ticket_sound = Label(
        frame_group_header, text=translate[language_code]['play_ticket_sound'])
    lbl_play_ticket_sound.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_play_ticket_sound
    chk_state_play_ticket_sound = BooleanVar()
    chk_state_play_ticket_sound.set(
        config_dict["advanced"]["play_sound"]["ticket"])

    global chk_play_ticket_sound
    chk_play_ticket_sound = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_play_ticket_sound)
    chk_play_ticket_sound.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_play_order_sound
    lbl_play_order_sound = Label(
        frame_group_header, text=translate[language_code]['play_order_sound'])
    lbl_play_order_sound.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_play_order_sound
    chk_state_play_order_sound = BooleanVar()
    chk_state_play_order_sound.set(
        config_dict["advanced"]["play_sound"]["order"])

    global chk_play_order_sound
    chk_play_order_sound = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_play_order_sound)
    chk_play_order_sound.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_play_sound_filename
    lbl_play_sound_filename = Label(
        frame_group_header, text=translate[language_code]['play_sound_filename'])
    lbl_play_sound_filename.grid(column=0, row=group_row_count, sticky=E)

    global txt_play_sound_filename
    txt_play_sound_filename_value = StringVar(
        frame_group_header, value=play_sound_filename)
    txt_play_sound_filename = Entry(
        frame_group_header, width=30, textvariable=txt_play_sound_filename_value)
    ...
    lbl_icon_play = Label(frame_group_header,
                          image=icon_play_img, cursor="hand2")
    ...
    global lbl_max_dwell_time
    lbl_max_dwell_time = Label(
        frame_group_header, text=translate[language_code]['max_dwell_time'])
    lbl_max_dwell_time.grid(column=0, row=group_row_count, sticky=E)

    global txt_max_dwell_time
    txt_max_dwell_time_value = StringVar(
        frame_group_header, value=config_dict["kktix"]["max_dwell_time"])
    txt_max_dwell_time = Entry(
        frame_group_header, width=30, textvariable=txt_max_dwell_time_value)
    txt_max_dwell_time.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_cityline_queue_retry
    lbl_cityline_queue_retry = Label(
        frame_group_header, text=translate[language_code]['cityline_queue_retry'])
    ...
    chk_state_cityline_queue_retry.set(
        config_dict["cityline"]["cityline_queue_retry"])

    global chk_cityline_queue_retry
    chk_cityline_queue_retry = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_cityline_queue_retry)
    chk_cityline_queue_retry.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_reset_browser_interval
    lbl_reset_browser_interval = Label(
        frame_group_header, text=translate[language_code]['reset_browser_interval'])
    lbl_reset_browser_interval.grid(column=0, row=group_row_count, sticky=E)

    global txt_reset_browser_interval
    txt_reset_browser_interval_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["reset_browser_interval"])
    txt_reset_browser_interval = Entry(
        frame_group_header, width=30, textvariable=txt_reset_browser_interval_value)
    txt_reset_browser_interval.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_proxy_server_port
    lbl_proxy_server_port = Label(
        frame_group_header, text=translate[language_code]['proxy_server_port'])
    lbl_proxy_server_port.grid(column=0, row=group_row_count, sticky=E)

    global txt_proxy_server_port
    txt_proxy_server_port_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["proxy_server_port"])
    txt_proxy_server_port = Entry(
        frame_group_header, width=30, textvariable=txt_proxy_server_port_value)
    txt_proxy_server_port.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_window_size
    lbl_window_size = Label(
        frame_group_header, text=translate[language_code]['window_size'])
    lbl_window_size.grid(column=0, row=group_row_count, sticky=E)

    global txt_window_size
    txt_window_size_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["window_size"])
    txt_window_size = Entry(frame_group_header, width=30,
                            textvariable=txt_window_size_value)
    txt_window_size.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_chrome_extension
    lbl_chrome_extension = Label(
        frame_group_header, text=translate[language_code]['chrome_extension'])
    ...
    global chk_chrome_extension
    chk_chrome_extension = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_chrome_extension)
    chk_chrome_extension.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_hide_some_image
    lbl_hide_some_image = Label(
        frame_group_header, text=translate[language_code]['hide_some_image'])
    ...
    global chk_hide_some_image
    chk_hide_some_image = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_hide_some_image)
    chk_hide_some_image.grid(column=1, row=group_row_count, sticky=W)

    global lbl_hide_some_image_recommand
    lbl_hide_some_image_recommand = Label(
        frame_group_header, text=translate[language_code]['recommand_enable'])
    lbl_hide_some_image_recommand.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_block_facebook_network
    lbl_block_facebook_network = Label(
        frame_group_header, text=translate[language_code]['block_facebook_network'])
    lbl_block_facebook_network.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_block_facebook_network
    chk_state_block_facebook_network = BooleanVar()
    chk_state_block_facebook_network.set(
        config_dict["advanced"]["block_facebook_network"])

    global chk_block_facebook_network
    chk_block_facebook_network = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_block_facebook_network)
    chk_block_facebook_network.grid(column=1, row=group_row_count, sticky=W)

    global lbl_block_facebook_network_recommand
    lbl_block_facebook_network_recommand = Label(
        frame_group_header, text=translate[language_code]['recommand_enable'])
    lbl_block_facebook_network_recommand.grid(
        column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_headless
    lbl_headless = Label(frame_group_header,
                         text=translate[language_code]['headless'])
    ...
    global chk_headless
    chk_headless = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_headless)
    chk_headless.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_verbose
    lbl_verbose = Label(frame_group_header,
                        text=translate[language_code]['verbose'])
    ...
    global chk_verbose
    chk_verbose = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_verbose)
    # maybe enable in future.
    ...
    global lbl_ocr_captcha
    lbl_ocr_captcha = Label(
        frame_group_header, text=translate[language_code]["ocr_captcha"])
    ...
    global chk_ocr_captcha
    chk_ocr_captcha = Checkbutton(
        frame_group_ddddocr_enable, text=translate[language_code]['enable'], variable=chk_state_ocr_captcha, command=showHideOcrCaptchaWithSubmit)
    ...
    global lbl_ocr_captcha_ddddocr_beta
    lbl_ocr_captcha_ddddocr_beta = Label(
        frame_group_header, text=translate[language_code]['ocr_captcha_ddddocr_beta'])
    ...
    global chk_ocr_captcha_ddddocr_beta
    chk_ocr_captcha_ddddocr_beta = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_ocr_captcha_ddddocr_beta)
    ...
    global lbl_ocr_captcha_force_submit
    lbl_ocr_captcha_force_submit = Label(
        frame_group_header, text=translate[language_code]['ocr_captcha_force_submit'])
    lbl_ocr_captcha_force_submit.grid(
        column=0, row=ocr_captcha_force_submit_index, sticky=E)

    global chk_state_ocr_captcha_force_submit
    chk_state_ocr_captcha_force_submit = BooleanVar()
    chk_state_ocr_captcha_force_submit.set(
        config_dict["ocr_captcha"]["force_submit"])

    global chk_ocr_captcha_force_submit
    chk_ocr_captcha_force_submit = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_ocr_captcha_force_submit)
...
def VerificationTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    global lbl_user_guess_string_description
    lbl_user_guess_string_description = Label(
        frame_group_header, text=translate[language_code]['user_guess_string'])
    lbl_user_guess_string_description.grid(
        column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_user_guess_string
    lbl_user_guess_string = Label(
        frame_group_header, text=translate[language_code]['local_dictionary'])
    ...
    txt_user_guess_string.insert(
        "1.0", config_dict["advanced"]["user_guess_string"].strip())

    group_row_count += 1

    global lbl_remote_url
    lbl_remote_url = Label(
        frame_group_header, text=translate[language_code]['remote_url'])
    ...
    lbl_icon_preview_text = Label(
        frame_group_header, image=icon_preview_text_img, cursor="hand2")
    lbl_icon_preview_text.image = icon_preview_text_img
    lbl_icon_preview_text.grid(column=2, row=group_row_count, sticky=W+N)
    lbl_icon_preview_text.bind(
        "<Button-1>", lambda e: btn_open_text_server_clicked())

    group_row_count += 1

    global lbl_online_dictionary_preview
    lbl_online_dictionary_preview = Label(
        frame_group_header, text=translate[language_code]['preview'])
    ...
    lbl_online_dictionary_preview_data.grid(
        column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_auto_guess_options
    lbl_auto_guess_options = Label(
        frame_group_header, text=translate[language_code]['auto_guess_options'])
    lbl_auto_guess_options.grid(column=0, row=group_row_count, sticky=E)

    global chk_state_auto_guess_options
    chk_state_auto_guess_options = BooleanVar()
    chk_state_auto_guess_options.set(
        config_dict["advanced"]["auto_guess_options"])

    global chk_auto_guess_options
    chk_auto_guess_options = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_auto_guess_options)
    chk_auto_guess_options.grid(column=1, row=group_row_count, sticky=W)

    group_row_count += 1

    # ==================== 新增「取消自動更新設定檔設定」開始 ====================
    global lbl_disable_auto_update_config
    # 這裡直接用中文，如果有多語系需求可以到 load_translate 增加
    lbl_disable_auto_update_config = Label(
        frame_group_header, text="取消自動更新設定檔設定")
    lbl_disable_auto_update_config.grid(
        column=0, row=group_row_count, sticky=E)

    global chk_state_disable_auto_update_config
    chk_state_disable_auto_update_config = BooleanVar()
    chk_state_disable_auto_update_config.set(
        config_dict["advanced"].get("disable_auto_update_config", False))

    global chk_disable_auto_update_config
    chk_disable_auto_update_config = Checkbutton(
        frame_group_header, text=translate[language_code]['enable'], variable=chk_state_disable_auto_update_config)
    chk_disable_auto_update_config.grid(
        column=1, row=group_row_count, sticky=W)
...
def ServerTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    global lbl_server_url
    lbl_server_url = Label(
        frame_group_header, text=translate[language_code]['server_url'])
    ...
    lbl_icon_copy_ip = Label(
        frame_group_header, image=icon_copy_img, cursor="hand2")
    ...
    global lbl_question
    lbl_question = Label(frame_group_header,
                         text=translate[language_code]['question'])
    ...
    lbl_icon_copy_question = Label(
        frame_group_header, image=icon_copy_img, cursor="hand2")
    lbl_icon_copy_question.image = icon_copy_img
    # lbl_icon_copy_question.grid(column=2, row=group_row_count, sticky = W+N)
    lbl_icon_copy_question.bind(
        "<Button-1>", lambda e: btn_copy_question_clicked())

    icon_query_filename = "icon_query_5.gif"
    icon_query_img = PhotoImage(file=icon_query_filename)

    lbl_icon_query_question = Label(
        frame_group_header, image=icon_query_img, cursor="hand2")
    lbl_icon_query_question.image = icon_query_img
    lbl_icon_query_question.grid(column=2, row=group_row_count, sticky=W+N)
    lbl_icon_query_question.bind(
        "<Button-1>", lambda e: btn_query_question_clicked())

    group_row_count += 1

    global lbl_answer
    lbl_answer = Label(frame_group_header,
                       text=translate[language_code]['answer'])
...
def AutofillTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    global lbl_tixcraft_sid
    lbl_tixcraft_sid = Label(
        frame_group_header, text=translate[language_code]['tixcraft_sid'])
    lbl_tixcraft_sid.grid(column=0, row=group_row_count, sticky=E)

    global txt_tixcraft_sid
    txt_tixcraft_sid_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["tixcraft_sid"].strip())
    txt_tixcraft_sid = Entry(frame_group_header, width=30,
                             textvariable=txt_tixcraft_sid_value, show="*")
    txt_tixcraft_sid.grid(column=1, row=group_row_count,
                          columnspan=2, sticky=W)

    group_row_count += 1

    global lbl_ibon_ibonqware
    lbl_ibon_ibonqware = Label(
        frame_group_header, text=translate[language_code]['ibon_ibonqware'])
    lbl_ibon_ibonqware.grid(column=0, row=group_row_count, sticky=E)

    global txt_ibon_ibonqware
    txt_ibon_ibonqware_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["ibonqware"].strip())
    txt_ibon_ibonqware = Entry(
        frame_group_header, width=30, textvariable=txt_ibon_ibonqware_value, show="*")
    txt_ibon_ibonqware.grid(
        column=1, row=group_row_count, columnspan=2, sticky=W)

    group_row_count += 1

    global lbl_password
    lbl_password = Label(frame_group_header,
                         text=translate[language_code]['password'])
    lbl_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_facebook_account
    lbl_facebook_account = Label(
        frame_group_header, text=translate[language_code]['facebook_account'])
    lbl_facebook_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_facebook_account
    txt_facebook_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["facebook_account"].strip())
    txt_facebook_account = Entry(
        frame_group_header, width=15, textvariable=txt_facebook_account_value)
    txt_facebook_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_facebook_password
    txt_facebook_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["facebook_password"].strip()))
    txt_facebook_password = Entry(
        frame_group_header, width=15, textvariable=txt_facebook_password_value, show="*")
    txt_facebook_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_kktix_account
    lbl_kktix_account = Label(
        frame_group_header, text=translate[language_code]['kktix_account'])
    lbl_kktix_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_kktix_account
    txt_kktix_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["kktix_account"].strip())
    txt_kktix_account = Entry(
        frame_group_header, width=15, textvariable=txt_kktix_account_value)
    txt_kktix_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_kktix_password
    txt_kktix_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["kktix_password"].strip()))
    txt_kktix_password = Entry(
        frame_group_header, width=15, textvariable=txt_kktix_password_value, show="*")
    txt_kktix_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_fami_account
    lbl_fami_account = Label(
        frame_group_header, text=translate[language_code]['fami_account'])
    lbl_fami_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_fami_account
    txt_fami_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["fami_account"].strip())
    txt_fami_account = Entry(frame_group_header, width=15,
                             textvariable=txt_fami_account_value)
    txt_fami_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_fami_password
    txt_fami_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["fami_password"].strip()))
    txt_fami_password = Entry(
        frame_group_header, width=15, textvariable=txt_fami_password_value, show="*")
    txt_fami_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_cityline_account
    lbl_cityline_account = Label(
        frame_group_header, text=translate[language_code]['cityline_account'])
    lbl_cityline_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_cityline_account
    txt_cityline_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["cityline_account"].strip())
    txt_cityline_account = Entry(
        frame_group_header, width=30, textvariable=txt_cityline_account_value)
    txt_cityline_account.grid(
        column=1, row=group_row_count, sticky=W, columnspan=2)

    global txt_cityline_password
    txt_cityline_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["cityline_password"].strip()))
    txt_cityline_password = Entry(
        frame_group_header, width=15, textvariable=txt_cityline_password_value, show="*")
    # txt_cityline_password.grid(column=2, row=group_row_count, sticky = W)

    group_row_count += 1

    global lbl_urbtix_account
    lbl_urbtix_account = Label(
        frame_group_header, text=translate[language_code]['urbtix_account'])
    lbl_urbtix_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_urbtix_account
    txt_urbtix_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["urbtix_account"].strip())
    txt_urbtix_account = Entry(
        frame_group_header, width=15, textvariable=txt_urbtix_account_value)
    txt_urbtix_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_urbtix_password
    txt_urbtix_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["urbtix_password"].strip()))
    txt_urbtix_password = Entry(
        frame_group_header, width=15, textvariable=txt_urbtix_password_value, show="*")
    txt_urbtix_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_hkticketing_account
    lbl_hkticketing_account = Label(
        frame_group_header, text=translate[language_code]['hkticketing_account'])
    lbl_hkticketing_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_hkticketing_account
    txt_hkticketing_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["hkticketing_account"].strip())
    txt_hkticketing_account = Entry(
        frame_group_header, width=15, textvariable=txt_hkticketing_account_value)
    txt_hkticketing_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_hkticketing_password
    txt_hkticketing_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["hkticketing_password"].strip()))
    txt_hkticketing_password = Entry(
        frame_group_header, width=15, textvariable=txt_hkticketing_password_value, show="*")
    txt_hkticketing_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_kham_account
    lbl_kham_account = Label(
        frame_group_header, text=translate[language_code]['kham_account'])
    lbl_kham_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_kham_account
    txt_kham_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["kham_account"].strip())
    txt_kham_account = Entry(frame_group_header, width=15,
                             textvariable=txt_kham_account_value)
    txt_kham_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_kham_password
    txt_kham_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["kham_password"].strip()))
    txt_kham_password = Entry(
        frame_group_header, width=15, textvariable=txt_kham_password_value, show="*")
    txt_kham_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_ticket_account
    lbl_ticket_account = Label(
        frame_group_header, text=translate[language_code]['ticket_account'])
    lbl_ticket_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_ticket_account
    txt_ticket_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["ticket_account"].strip())
    txt_ticket_account = Entry(
        frame_group_header, width=15, textvariable=txt_ticket_account_value)
    txt_ticket_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_ticket_password
    txt_ticket_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["ticket_password"].strip()))
    txt_ticket_password = Entry(
        frame_group_header, width=15, textvariable=txt_ticket_password_value, show="*")
    txt_ticket_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_udn_account
    lbl_udn_account = Label(
        frame_group_header, text=translate[language_code]['udn_account'])
    lbl_udn_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_udn_account
    txt_udn_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["udn_account"].strip())
    txt_udn_account = Entry(frame_group_header, width=15,
                            textvariable=txt_udn_account_value)
    txt_udn_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_udn_password
    txt_udn_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["udn_password"].strip()))
    txt_udn_password = Entry(frame_group_header, width=15,
                             textvariable=txt_udn_password_value, show="*")
    txt_udn_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_ticketplus_account
    lbl_ticketplus_account = Label(
        frame_group_header, text=translate[language_code]['ticketplus_account'])
    lbl_ticketplus_account.grid(column=0, row=group_row_count, sticky=E)

    global txt_ticketplus_account
    txt_ticketplus_account_value = StringVar(
        frame_group_header, value=config_dict["advanced"]["ticketplus_account"].strip())
    txt_ticketplus_account = Entry(
        frame_group_header, width=15, textvariable=txt_ticketplus_account_value)
    txt_ticketplus_account.grid(column=1, row=group_row_count, sticky=W)

    global txt_ticketplus_password
    txt_ticketplus_password_value = StringVar(frame_group_header, value=util.decrypt_me(
        config_dict["advanced"]["ticketplus_password"].strip()))
    txt_ticketplus_password = Entry(
        frame_group_header, width=15, textvariable=txt_ticketplus_password_value, show="*")
    txt_ticketplus_password.grid(column=2, row=group_row_count, sticky=W)

    group_row_count += 1

    global lbl_save_password_alert
    lbl_save_password_alert = Label(
        frame_group_header, fg="red", text=translate[language_code]['save_password_alert'])
    lbl_save_password_alert.grid(
        column=0, row=group_row_count, columnspan=2, sticky=E)

    frame_group_header.grid(column=0, row=row_count, padx=UI_PADDING_X)

    # ==================== [新增] Telegram 設定介面 ====================
    group_row_count += 1  # 換行分隔

    # 0. 啟用開關 (Checkbox)
    global chk_state_telegram_enable
    chk_state_telegram_enable = BooleanVar()
    # 讀取設定，預設為 False
    chk_state_telegram_enable.set(
        config_dict["advanced"].get("telegram_enable", False))

    global chk_telegram_enable
    # 這裡的 text 可以改成翻譯變數，或直接寫中文
    chk_telegram_enable = Checkbutton(
        frame_group_header,
        text="啟用 Telegram 通知 (Enable Telegram Notify)",
        variable=chk_state_telegram_enable
    )
    # 讓勾選框跨兩欄靠左對齊
    chk_telegram_enable.grid(
        column=0, row=group_row_count, columnspan=2, sticky=W)

    group_row_count += 1  # 換行

    # 1. Telegram Bot Token (保持原樣，或是加上狀態連動)
    global txt_telegram_bot_token
    lbl_telegram_bot_token = Label(
        frame_group_header, text="Telegram Bot Token:")
    lbl_telegram_bot_token.grid(column=0, row=group_row_count, sticky=E)

    val_token = config_dict["advanced"].get("telegram_bot_token", "")
    txt_telegram_bot_token_value = StringVar(
        frame_group_header, value=val_token)
    txt_telegram_bot_token = Entry(
        frame_group_header, width=40, textvariable=txt_telegram_bot_token_value)
    txt_telegram_bot_token.grid(
        column=1, row=group_row_count, columnspan=2, sticky=W)

    group_row_count += 1  # 換行

    # 2. Telegram Chat ID (保持原樣)
    global txt_telegram_chat_id
    lbl_telegram_chat_id = Label(frame_group_header, text="Telegram Chat ID:")
    lbl_telegram_chat_id.grid(column=0, row=group_row_count, sticky=E)

    val_chat_id = config_dict["advanced"].get("telegram_chat_id", "")
    txt_telegram_chat_id_value = StringVar(
        frame_group_header, value=val_chat_id)
    txt_telegram_chat_id = Entry(
        frame_group_header, width=40, textvariable=txt_telegram_chat_id_value)
    txt_telegram_chat_id.grid(
        column=1, row=group_row_count, columnspan=2, sticky=W)
...
def change_maxbot_status_by_keyword():
    ...
    import util
    app_root = util.get_app_root()
    lock_file = os.path.join(app_root, "MAXBOT_002_LOCK.txt")
    ...
    if os.path.exists(lock_file):
        try:
            ...
            # 如果現在時間小於過期時間，代表還在 002 保護期內，忽略暫停/接續指令
    ...
    if selected_tab_index == 0:
        ...
        try:
            ...
            if 'txt_date_keyword' in globals():
                date_keyword = txt_date_keyword.get("1.0", END).strip()
                date_keyword = util.format_config_keyword_for_json(
                    date_keyword)

            area_keyword = ""
            if 'txt_area_keyword' in globals():
                area_keyword = txt_area_keyword.get("1.0", END).strip()
                area_keyword = util.format_config_keyword_for_json(
                    area_keyword)

            keyword_exclude = ""
            if 'txt_keyword_exclude' in globals():
                keyword_exclude = txt_keyword_exclude.get("1.0", END).strip()
                keyword_exclude = util.format_config_keyword_for_json(
                    keyword_exclude)
            ...
            if txt_date_keyword_highlightthickness != highlightthickness:
                ...
                txt_date_keyword.config(
                    highlightthickness=highlightthickness, highlightbackground="red")
    ...
    if selected_tab_index == 5:
        ...
        try:
            ...
            if 'txt_idle_keyword' in globals():
                idle_keyword = txt_idle_keyword.get("1.0", END).strip()
                idle_keyword = util.format_config_keyword_for_json(
                    idle_keyword)

            resume_keyword = ""
            if 'txt_resume_keyword' in globals():
                resume_keyword = txt_resume_keyword.get("1.0", END).strip()
                resume_keyword = util.format_config_keyword_for_json(
                    resume_keyword)

            idle_keyword_second = ""
            if 'txt_idle_keyword_second' in globals():
                idle_keyword_second = txt_idle_keyword_second.get(
                    "1.0", END).strip()
                idle_keyword_second = util.format_config_keyword_for_json(
                    idle_keyword_second)
...
def RuntimeTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    maxbot_status = ""
    global lbl_maxbot_status
    lbl_maxbot_status = Label(
        frame_group_header, text=translate[language_code]['running_status'])
    ...
    btn_idle = ttk.Button(frame_maxbot_interrupt,
                          text=translate[language_code]['idle'], command=lambda: btn_idle_clicked(language_code))
    btn_idle.grid(column=1, row=0)

    btn_resume = ttk.Button(
        frame_maxbot_interrupt, text=translate[language_code]['resume'], command=lambda: btn_resume_clicked(language_code))
    btn_resume.grid(column=2, row=0)
    ...
    global lbl_maxbot_last_url
    lbl_maxbot_last_url = Label(
        frame_group_header, text=translate[language_code]['running_url'])
    ...
    global lbl_system_clock
    lbl_system_clock = Label(
        frame_group_header, text=translate[language_code]['system_clock'])
    ...
    global lbl_idle_keyword
    lbl_idle_keyword = Label(
        frame_group_header, text=translate[language_code]['idle_keyword'])
    ...
    global lbl_resume_keyword
    lbl_resume_keyword = Label(
        frame_group_header, text=translate[language_code]['resume_keyword'])
    ...
    global lbl_idle_keyword_second
    lbl_idle_keyword_second = Label(
        frame_group_header, text=translate[language_code]['idle_keyword_second'])
    ...
    global lbl_resume_keyword_second
    lbl_resume_keyword_second = Label(
        frame_group_header, text=translate[language_code]['resume_keyword_second'])
...
def AboutTab(root, language_code):
    ...
    lbl_slogan = Label(
        frame_group_header, text=translate[language_code]['maxbot_slogan'], wraplength=400, justify="center")
    ...
    lbl_help_url = Label(frame_group_header, text=URL_HELP,
                         fg="blue", bg="gray", cursor="hand2")
    lbl_help_url.grid(column=1, row=group_row_count, sticky=W)
    lbl_help_url.bind("<Button-1>", lambda e: open_url(URL_HELP))

    group_row_count += 1

    lbl_donate = Label(frame_group_header,
                       text=translate[language_code]['donate'])
    lbl_donate.grid(column=0, row=group_row_count, sticky=E)

    lbl_donate_url = Label(frame_group_header, text=URL_DONATE,
                           fg="blue", bg="gray", cursor="hand2")
    ...
    lbl_release = Label(frame_group_header,
                        text=translate[language_code]['release'])
    lbl_release.grid(column=0, row=group_row_count, sticky=E)

    lbl_release_url = Label(
        frame_group_header, text=URL_RELEASE, fg="blue", bg="gray", cursor="hand2")
    ...
    lbl_fb_fans_url = Label(frame_group_header, text=URL_FB,
                            fg="blue", bg="gray", cursor="hand2")
    ...
    lbl_chrome_driver_url = Label(
        frame_group_header, text=URL_CHROME_DRIVER, fg="blue", bg="gray", cursor="hand2")
    lbl_chrome_driver_url.grid(column=1, row=group_row_count, sticky=W)
    lbl_chrome_driver_url.bind(
        "<Button-1>", lambda e: open_url(URL_CHROME_DRIVER))
    ...
    lbl_firefox_driver_url = Label(
        frame_group_header, text=URL_FIREFOX_DRIVER, fg="blue", bg="gray", cursor="hand2")
    lbl_firefox_driver_url.grid(column=1, row=group_row_count, sticky=W)
    lbl_firefox_driver_url.bind(
        "<Button-1>", lambda e: open_url(URL_FIREFOX_DRIVER))
    ...
    lbl_edge_driver_url = Label(
        frame_group_header, text=URL_EDGE_DRIVER, fg="blue", bg="gray", cursor="hand2")
...
def get_action_bar(root, language_code):
    ...
    btn_run = ttk.Button(
        frame_action, text=translate[language_code]['run'], command=btn_run_clicked)
    btn_run.grid(column=0, row=0)

    btn_save = ttk.Button(
        frame_action, text=translate[language_code]['save'], command=btn_save_clicked)
    btn_save.grid(column=1, row=0)

    btn_exit = ttk.Button(
        frame_action, text=translate[language_code]['exit'], command=btn_exit_clicked)
    # btn_exit.grid(column=2, row=0)

    btn_launcher = ttk.Button(
        frame_action, text=translate[language_code]['config_launcher'], command=btn_launcher_clicked)
    btn_launcher.grid(column=2, row=0)

    btn_restore_defaults = ttk.Button(
        frame_action, text=translate[language_code]['restore_defaults'], command=btn_restore_defaults_clicked)
...
def main_gui():
    ...
    iconImg = ''
    if platform.system() == 'Linux':
...
def clean_tmp_file():
    remove_file_list = [CONST_MAXBOT_LAST_URL_FILE, CONST_MAXBOT_INT28_FILE,
                        CONST_MAXBOT_ANSWER_ONLINE_FILE, CONST_MAXBOT_QUESTION_FILE, "MAXBOT_002_LOCK.txt"]
    ...
    if os.path.exists(CONST_MAXBOT_QUESTION_FILE):
        ...
        if not infile is None:
            ...
            if 'txt_question' in globals():
                try:
                    displayed_question_text = txt_question.get(
                        "1.0", END).strip()
